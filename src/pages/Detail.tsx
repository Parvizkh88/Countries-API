    ...
    <div>
      {countrySearched && (
        <>
          ...
          <Card
            style={{ width: "18rem", marginLeft: "35rem", marginTop: "10rem" }}
          >
            <Card.Img variant="top" src={countrySearched[0]?.flags.svg} />
            <Card.Body>
              <Card.Title className="custom-card">
                {countrySearched[0]?.name.official}
              </Card.Title>
              <Card.Subtitle className="mb-2 text-muted">
                {countrySearched[0]?.region}
              </Card.Subtitle>
              <Card.Text className="custom-card">
                This country is in <b>{countrySearched[0]?.region}</b> continent
                and its population is <b>{countrySearched[0]?.population}</b>{" "}
                people. example text to build on the card title and make up the
